...
USER_AGENT_STRING = str.encode('%s-%s/%s (%s;%s-%s)' % (PICARD_ORG_NAME, PICARD_APP_NAME,
                                             PICARD_VERSION_STR,
                                             platform.platform(),
...
class XmlWebService(QtCore.QObject):
    ...
    def set_cache(self, cache_size_in_mb=100):
        ...
        cache.setCacheDirectory(os.path.join(str(location), u'picard'))
    ...
    def _start_request_continue(self, method, host, port, path, data, handler, xml,
        ...
        if mblogin and access_token:
            request.setRawHeader(b"Authorization", b"Bearer %s" % access_token)
        if mblogin or (method == "GET" and refresh):
            ...
            request.setAttribute(QtNetwork.QNetworkRequest.CacheLoadControlAttribute,
                                 cacheloadcontrol)
        request.setRawHeader(b"User-Agent", USER_AGENT_STRING)
        if xml:
